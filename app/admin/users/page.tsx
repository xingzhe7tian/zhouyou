...
export default function UserManagement() {
  ...
  useEffect(() => {
    if (typeFromUrl) {
      setUserTypeFilter(typeFromUrl === 'normal' ? '普通用户' : typeFromUrl === 'gm' ? 'GM用户' : 'all')
    }
  }, [typeFromUrl])
  ...
  const handleUserTypeChange = (value: string) => {
    ...
    // 更新 URL，但不触发页面刷新
    const newUrl = value === 'all' ? '/admin/users' : `/admin/users?type=${value === '普通用户' ? 'normal' : 'gm'}`
    ...
    <Card>
      ...
      <CardContent>
        <div className="flex justify-between mb-4">
          <Select onValueChange={handleUserTypeChange} value={userTypeFilter}>
